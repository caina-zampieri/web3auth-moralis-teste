...
export function useWeb3AuthSave(){
  
  const { authenticate, authError, isAuthenticating, Moralis } = useMoralis();


  const { provider, user, login, logout, getUserInfo, getAccounts, getBalance, signMessage } = useWeb3Auth();

  const  Web3SaveContext = createContext({
   save: async () => {
      ...
      monster.set("strength", getUserInfo());
      monster.set("ownerName", "getUserInfo");
  ...
  return useContext(Web3SaveContext)
}
