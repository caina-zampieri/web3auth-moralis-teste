...
const TORUS_DIRECT_SDK_VERIFIER_NAME = "web3auth-meu-firebase-demo";
...
function getUiConfig() {
  return {
    ...
    credentialHelper:
      CLIENT_ID && CLIENT_ID != "825329295126-vmjfcondbi3knev0hbj65h9ffbe4mo5t.apps.googleusercontent.com" ? firebaseui.auth.CredentialHelper.GOOGLE_YOLO : firebaseui.auth.CredentialHelper.NONE,
  };
}
 ...
 */
var handleSignedInUser = function (user, privateKey, publicAddress) {
  document.getElementById("user-signed-in").style.display = "block";
  document.getElementById("user-signed-out").style.display = "none";

  document.getElementById("privateKey").textContent = "Private Key: " + privateKey;
  document.getElementById("publicKey").textContent = "Public Address: " + publicAddress;
...
function handleConfigChange() {
  ...
  var newEmailSignInMethodValue = document.querySelector('input[name="emailSignInMethod"]:checked').value;
  var currentDisableSignUpStatus = document.getElementById("email-disableSignUp-status").checked;
  var currentAdminRestrictedOperationStatus = document.getElementById("admin-restricted-operation-status").checked;
  location.replace(
    location.pathname +
      "#recaptcha=" +
      newRecaptchaValue +
